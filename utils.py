...
def input_setup(sess, config, data_dir, index=0):
    ...
    if config.is_train:
        for i in range(len(data)):
            ...
            input_ = imread(data[i]) / 255 #(imread(data[i]) - 127.5) / 127.5#
        ...
        input_ = imread(data[index]) / 255 #(imread(data[index]) - 127.5) / 127.5#
        if len(input_.shape) == 3:
...
def gradient(input):
    ...
    Gradient = tf.abs(Gradient1) + tf.abs(Gradient2)#tf.sqrt(tf.square(Gradient1) + tf.square(Gradient2))#
    # print(d)
